...
Future<VideoDetails?> getVideo(String url, Function changeProgress) async {
  ...
  Response response = await get(
    Uri.parse('https://animesuge.io/ajax/anime/servers?id=' +
        id +
        '&episode=' +
  ...
  String frameURL = decodeIFrameUrl(
  ...
  String videoURL = '';
  for (String i in web.getElementTitle('script')) {
    if (i.startsWith('document.getElementById(')) {
      List<String> splicedUrl = i.split(
        RegExp("(\"|')"),
      );
      int index = int.parse(
        i[i.lastIndexOf("(") + 1],
      );
      videoURL = "https://streamtape.com/" +
          splicedUrl[3].split('.com/')[1] +
          splicedUrl[5].substring(index);
    }
  }
